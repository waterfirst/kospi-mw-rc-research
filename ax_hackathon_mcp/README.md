...
| `daily_workflow` | 07:30 시가 예측부터 마감 복기까지 하루 운영 루틴 반환 |
| `submission_answers` | AX 해커톤 제출 문항 답변 반환 |

## 하루 운영 루틴

이 MCP는 단발성 질의뿐 아니라 하루 장 운영 루틴에도 맞게 설계했습니다. 단, MCP가 직접 카카오톡을 보내거나 스케줄러를 점유하는 방식이 아니라, **MCP는 판단 결과와 메시지 payload를 만들고 카카오페이증권 앱/카카오톡 채널/사내 봇이 전송을 담당**하는 구조가 적절합니다.

| 시각(KST) | 동작 | 사용자에게 전달할 내용 |
|---|---|---|
| 07:30 | 시가 예측 | 장전 레짐, 예상 범위, 미국장/EWY/반도체/뉴스 근거 |
| 09:05 | 시가 채점 + 1차 모니터링 | 갭 성공/실패, 초기 외국인·기관·프로그램 흐름 |
| 10:30 | 2차 모니터링 | 수급 변화, 종가 예비 레짐, 리스크 플래그 |
| 12:30 | 종가 예측 고정 | 종가 예상 범위, confidence, 기관 흡수/매도 애벌랜치 여부 |
| 장마감 후 | 점수·복기·수정 | 오차, 점수, 틀린 원인, 다음 모델 수정 방향 |

실전 검증 중에는 Telegram으로 자동 전송했지만, 카카오페이증권 서비스에서는 카카오톡 채널 메시지, 앱 푸시, 내부 리서치 봇으로 바꾸면 됩니다.
